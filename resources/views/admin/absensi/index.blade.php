...
<div class="main">
    ...
    <form method="GET" class="filter-section mb-2 d-flex align-items-center justify-content-center gap-2">
    <input type="date" name="date" value="{{ $filterDate }}" />
    <label for="perPage" class="mb-0 ms-2">Show</label>
    <input type="number" min="1" max="100" name="perPage" id="perPage" value="{{ $perPage }}" style="width:70px;">
    <span class="me-2">entries</span>
    <button type="submit">Filter</button>
    @if($filterDate || $perPage != 10)
        <a href="{{ route('admin.absensi') }}" class="btn btn-link">Reset</a>
    @endif
    </form>
        ...
        <table class="table table-bordered align-middle">
            ...
            @forelse ($attendances as $a)
                <tr>
                    <td>{{ $a->user->id ?? '-' }}</td>
                    <td>{{ $a->user->name ?? '-' }}</td>
                    <td>{{ \Carbon\Carbon::parse($a->schedule->meeting_date ?? $a->checkin_time)->format('d/m/Y') }}</td>
                    <td>
